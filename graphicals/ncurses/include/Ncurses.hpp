...
namespace Arcade
{
    class Ncurses : public IGraphicals {
        public:
            ...
            void checkButtons(int i);

            void createBox(int y1, int x1, int y2, int x2);

        private:
            std::string _title = "Game";
            std::string _username = "Prince";
            std::string _libChosen = "NONE";
            std::string _gameChosen = "NONE";
            std::size_t _height;
            std::size_t _width;
            std::string _score;
            ...
            std::map<int, void (Arcade::Ncurses::*)()> _sceneFunc;
            std::map<int, void (Arcade::Ncurses::*)()> _menuFunc;
            WINDOW *_window;
            std::array<std::string, 4> _menuString = {"PLAY", "HOW TO PLAY", "SETTINGS", "QUIT"};

            std::map<int, Arcade::EventKey> _keysTemplate = {
                {10, EventKey::ENTER},
                {97, EventKey::A},
                {98, EventKey::B},
                {99, EventKey::C},
                ...
                {27, EventKey::ESCAPE},
                {KEY_UP, EventKey::UP},
                {KEY_DOWN, EventKey::DOWN},
                {KEY_LEFT, EventKey::LEFT},
                {KEY_RIGHT, EventKey::RIGHT},
            };
    };
}